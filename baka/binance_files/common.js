...
function shouldShowDisclaimerTopBar() {
  ...
  const RULES_BLACK_LIST_PATH = ['/square', '/game']
  // only show except the rules in the black list
  const shouldBeHiddenByRules = RULES_BLACK_LIST_PATH.some(rule => window.location.href.includes(rule))

  const { region = '', locale = '', disclaimerConfig = {} } = bnvelidate || {}
  const { enable = false } = disclaimerConfig

  if (!shouldBeHiddenByRules && region.toLowerCase() === TARGET_REGION && locale === TARGET_LNG) {
    return true
  }
